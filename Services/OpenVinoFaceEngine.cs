...
using OpenCvSharp;
using OpenVinoSharp;
// MAUI 隐式引用了 Microsoft.Maui.Graphics.Rect，与 OpenCvSharp.Rect 冲突，显式别名消歧。
using Rect = OpenCvSharp.Rect;

namespace VisitorApp.Services;

/// <summary>
/// OpenVINO 人脸引擎（detect 检测 / extract 识别 / quality 姿态，IR 格式模型）。
/// 移植自 FaceCompareTest 已验证的实现：
/// read_model → reshape[1,3,H,W] → compile_model(设备) → set_input_tensor → infer → get_output_tensor。
/// 非 CPU 设备初始化失败时自动回退 CPU（见 Init）。
/// </summary>
public sealed class OpenVinoFaceEngine : IDisposable
{
    // ===== 预处理参数（已在 FaceCompareTest 中实测验证）=====
    private const int DetSize = 640;                 // detect 输入
    private const float DetScale = 1f / 128f;        // 检测：scale=1/128, mean=0, RGB
    private const int RecSize = 112;                 // extract 输入
    private const float RecStd = 127.5f;             // 识别：(x-127.5)/127.5, RGB
    ...
    /// <summary>实际使用的推理设备（请求的设备不可用时为回退后的设备）。</summary>
    ...
    /// <summary>初始化三个模型。device: CPU / GPU / AUTO（AUTO 映射为 AUTO:GPU,CPU）。</summary>
    public void Init(string modelsDir, string device)
    {
        var target = device == FaceInferenceDeviceStore.Auto ? "AUTO:GPU,CPU" : device;
        try
        {
            InitCore(modelsDir, target);
            Device = device;
        }
        catch (Exception ex) when (target != FaceInferenceDeviceStore.Cpu)
        {
            FallbackNote = $"推理设备 {device} 不可用（{FirstLine(ex.Message)}），已自动回退 CPU";
            DisposeCore();
            InitCore(modelsDir, FaceInferenceDeviceStore.Cpu);
            Device = FaceInferenceDeviceStore.Cpu;
        }
    }
    ...
    private (CompiledModel compiled, InferRequest request) Load(string modelsDir, string name, int w, int h, string device)
    {
        ...
        if (!File.Exists(xml) || !File.Exists(bin))
            throw new FileNotFoundException($"缺少人脸模型文件：{name}.xml / {name}.bin（应位于 {modelsDir}）");
    ...
    public FaceInfo? Process(Mat img, bool withQuality)
    {
        // ===== 1. 检测 =====
